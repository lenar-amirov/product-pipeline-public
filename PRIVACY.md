...
Product Discovery is a **local-first** Claude Code tool. It runs entirely on your machine. There is no server, no cloud backend, no analytics endpoint, no telemetry. We — the maintainers — do not collect, store, or transmit your data. Anywhere.

The rest of this document explains what that means in practice and what third parties (Anthropic, GitHub, optional MCP servers you choose to connect) do receive when you use the tool.

---

## What stays on your machine

Everything you produce while using the tool lives in your local working directory (your clone of this repository):
...
These files never leave your machine through the tool itself. They sit on your local filesystem, and the repository's .gitignore plus the optional pre-push leak guard (tools/scripts/check-leaks.py) are designed to keep them out of git. If you choose to commit them to a private repo of your own, that's your decision and goes to your git provider — not us.

---

## What goes to Anthropic

When you use Claude Code (which the tool runs inside), the conversation between you and Claude is processed by **Anthropic**. This includes:
...
This is governed by **your contract with Anthropic**, not by this tool. See:
- [Anthropic Privacy Policy](https://www.anthropic.com/privacy)
- [Claude Code Documentation](https://docs.anthropic.com/en/docs/claude-code)

The tool does not modify what Claude Code sends to Anthropic.
...
- **Install** — `git clone` (or Download ZIP) of this public repository. GitHub sees the clone/download request, same as for any public repo.
- **Updates** — `git pull`; GitHub sees the fetch request.
...
The tool supports — but does not require — these integrations:
...
You install these MCP servers separately and you control their credentials (in gitignored `.mcp.json`). The tool only invokes them when relevant jobs run, with data you've already authored locally. We do not proxy any of this through our own servers — there are no servers.
...
**Plain summary in one paragraph**: this tool runs locally and does not collect data. Anthropic processes the conversation you have with Claude Code. GitHub sees that you cloned a public repo. Anything else in your filesystem stays in your filesystem. Public Issues and Discussions are public.
